...
/// scope. Also rejects an integer (scalar or vector-element) wider than 64
/// bits: gmir.constant always stores its value in a 64-bit attribute (see
/// its doc comment), so a wider !gmir.llt would make the translator's later
/// truncate-back-down step truncate *up*, which APInt::trunc() forbids and
/// asserts on. Rejecting it here, at the single shared type-conversion
/// choke point, also correctly rejects non-constant wide-integer values
/// (e.g. two real i128 arguments added together), which nothing downstream
/// validates either. Note: both `Value` and `Type` name distinct classes in
...
static gmir::LLTType convertType(mlir::MLIRContext &Context, llvm::Type *Ty) {
  ...
  if (auto *PtrTy = dyn_cast<llvm::PointerType>(Ty))
    return gmir::LLTType::get(&Context, /*scalarSizeInBits=*/0,
                              ...
                              /*isScalable=*/false);
  // Fixed-width integer vectors only. The plain `dyn_cast<IntegerType>`
  // below already safely rejects vector-of-pointer (VectorType::
  // isValidElementType permits `<N x ptr>`, but !gmir.llt has no notion of
  // "vector of pointer" -- only scalarSizeInBits==0 means pointer, which a
  // vector shape can't also express) and vector-of-float, without needing
  // an explicit extra check.
...
public:
  FunctionImporter(mlir::MLIRContext &Context, gmir::CallInstMap &CallInsts)
      : Context(Context), Builder(&Context), CallInsts(CallInsts) {}
...
private:
  ...
  /// are conservatively out of scope for now -- see GMIRDialect.td's
  /// gmir.call doc comment). Arguments flatten via computeGMIRLeafTypes/
  /// getOperands, the same mechanism as aggregate load/store. Records &CI
  /// into CallInsts, the only way MLIRToGMIRTranslator can recover the
  /// original llvm::CallInst it needs for CallLowering::lowerCall's
  /// CallBase-taking overload -- see GMIRDialect.td's gmir.call doc
  /// comment for why that overload is used instead of hand-building a
  /// CallLoweringInfo, and CallInstMap's doc comment (GMIRImporter.h) for
  /// why this is a plain out-of-band map instead of an mlir::OpaqueLoc.
...
mlir::func::FuncOp gmir::importFunction(mlir::ModuleOp Module, Function &F,
  ...
  if (!FunctionImporter(Context, CallInsts).import(F, FuncOp)) {
    FuncOp.erase();
    return {};
  }
  return FuncOp;
