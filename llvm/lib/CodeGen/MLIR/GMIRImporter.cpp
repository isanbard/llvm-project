...
static gmir::LLTType convertType(MLIRContext &Context, llvm::Type *Ty) {
...
static bool computeGMIRLeafTypes(MLIRContext &Context,
...
public:
  FunctionImporter(MLIRContext &Context)
...
/// Returns false the moment an unsupported construct is seen.
bool FunctionImporter::import(Function &F, func::FuncOp FuncOp) {
...
bool FunctionImporter::importBinaryOp(BinaryOperator &BinOp) {
...
bool FunctionImporter::importCondBr(CondBrInst &Br) {
  mlir::Value Cond;
...
bool FunctionImporter::importICmp(ICmpInst &ICmp) {
  ...
  auto Op = gmir::ICmpOp::create(
      ...
      Builder.getI64IntegerAttr(static_cast<int64_t>(ICmp.getPredicate())), LHS,
      RHS);
  ValueMap[&ICmp] = {Op.getResult()};
  return true;
}

bool FunctionImporter::importReturn(ReturnInst &Ret) {
...
bool FunctionImporter::importUncondBr(UncondBrInst &Br) {
...
/// Static-only (§1.5 of the design doc): dynamic-sized or non-entry-block
/// allocas fall back to the legacy selector rather than being modeled.
bool FunctionImporter::importAlloca(AllocaInst &AI) {
  ...
  auto Op =
      gmir::AllocaOp::create(Builder, Builder.getUnknownLoc(), ResTy,
...
/// plain scalar/pointer load is just the trivial single-leaf,
/// zero-offset case of the same code path.
bool FunctionImporter::importLoad(LoadInst &LI) {
...
/// Symmetric to importLoad: flattens an aggregate-typed store's value
/// operand into one gmir.store per leaf.
bool FunctionImporter::importStore(StoreInst &SI) {
  SmallVector<gmir::LLTType, 1> LeafTypes;
  SmallVector<uint64_t, 1> Offsets;
...
/// for a ~50-line algorithm that changes rarely. Not attempted here;
/// worth reconsidering if it ever proves to actually drift.
bool FunctionImporter::importGEP(GetElementPtrInst &GEP) {
...
/// overload -- see GMIRDialect.td's gmir.call doc comment for why that
/// overload is used instead of hand-building a CallLoweringInfo.
bool FunctionImporter::importCall(CallInst &CI) {
  ...
  for (llvm::Value *Arg : CI.args()) {
    SmallVector<gmir::LLTType, 1> LeafTypes;
    SmallVector<uint64_t, 1> Offsets;
    if (!computeGMIRLeafTypes(Context, *DL, Arg->getType(), LeafTypes, Offsets))
      return false;
...
/// already-valid object, not a user GEP, so it always gets the strong
/// nuw+inbounds guarantee regardless of anything else).
mlir::Value FunctionImporter::materializeLeafPtr(mlir::Value BasePtr,
                                                 gmir::LLTType PtrTy,
                                                 gmir::LLTType OffsetTy,
                                                 uint64_t Offset) {
  if (Offset == 0)
    return BasePtr;
...
/// value for this specific predecessor edge, in the same `BB.phis()`
/// order the block arguments were created in during pass 1.
bool FunctionImporter::getSuccessorOperands(
...
/// ConstantInt is -- a ConstantInt too wide for 64 bits, or some other
/// unmapped/unsupported value).
bool FunctionImporter::getOperands(llvm::Value *V,
...
/// types are always exactly one leaf, so this asserts via the size
/// check rather than silently truncating).
bool FunctionImporter::getScalarOperand(llvm::Value *V, mlir::Value &Out) {
