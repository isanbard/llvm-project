...
bool TargetPassConfig::addCoreISelPasses() {
  ...
  if (Selector == SelectorType::FastISel) {
    ...
    // MLIRISel has no abort-mode flag of its own (decision #6 only wants
    // graceful fallback), so force this to Disable rather than leaving it
    // at its default (Enable). Without this, some targets' GlobalISel pass
  ...
  // translator emits, so GMIRLegalizerInfoAdapter isn't needed for this to
  // work correctly.
  ...
  // no abort-mode flag of its own (decision #6 only calls for graceful
  // fallback), so it always resets rather than aborting -- redundant with
  // GlobalISelAbort already being forced to Disable above, but explicit
