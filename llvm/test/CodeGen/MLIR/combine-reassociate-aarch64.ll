...
;
; AArch64 variant of combine-identities.ll's constant-reassociation and
; repeated-operand additions (see that file for the full rationale on the
; ported rules themselves: ReassociateConstOpPattern,
; RepeatedOperandIdempotentPattern, XorSelfCancelPattern, all in
; GMIRCombiner.cpp). Unlike combine-identities-aarch64.ll, this is a
; separate dedicated file rather than more RUN lines there: confirmed
; empirically (`diff` against plain -global-isel) that most of these
; functions' mlir-isel output genuinely diverges from plain -global-isel
; on AArch64 -- AArch64O0PreLegalizerCombiner does constant reassociation
; for add/and independently, but not for mul/or/xor, nor the
; repeated-operand/self-cancel identities; splitting the two that happen
; to byte-match out to stay diffable while the rest sit elsewhere would
; fragment one coherent set of tests for no real benefit. A file-wide
; byte-diff RUN line would fail here the same way it would have for
; combine-disjoint-add-aarch64.ll's add_disjoint_and. Uses the same GMIR
; + ASM FileCheck pair for the same reason.
...
; x^x->0 identity (see combine-identities.ll), reducing %n to %b here
; (via XorSelfCancelPattern above) rewires %r's operands to (b, b) -- a
; live x^x that nothing folds away. With the identity present, the whole
; chain collapses to a constant. Confirmed empirically that this
; genuinely diverges from plain -global-isel on AArch64 too (its
; combiner leaves a real `eor w0, w8, w8` rather than folding to zero),
; same reason the rest of this file avoids the byte-diff oracle.
