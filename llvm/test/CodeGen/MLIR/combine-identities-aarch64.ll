...
; that GMIRCombiner's own fold()s did this work: AArch64PassConfig runs a
; real GICombiner pass (AArch64O0PreLegalizerCombiner) even at the
; -O0-equivalent opt level GMIRCombiner itself targets, gated on
; isGlobalISelOptNone() -- the same check that behaves differently under
; -enable-mlir-isel than under -global-isel elsewhere in this suite (see
; gep-aarch64.ll). So the real downstream combiner could in principle be
; doing these simplifications "for free," the same masking risk the real
; target Legalizer poses for legalizer tests (see narrow-scalar-i686.ll).
; The -print-gmir-after-combine FileCheck below is the actual proof here
; -- same CHECK patterns as combine-identities.ll.
...
; MulNegOneToSubPattern (GMIRCombiner.cpp) is this file's first case
; exercising a genuine OpRewritePattern, not a fold() -- see
; combine-identities.ll for the full rationale. Unlike that file's
; mul_neg_one, this one's byte-diff RUN lines above DO hold here:
; confirmed empirically that AArch64's real O0-equivalent combiner
; (AArch64O0PreLegalizerCombiner) already does this exact
...
; Three new SubOp::fold arms (A-(A-B)->B, (A+B)-A->B, (A+B)-B->A -- see
; combine-identities.ll). Tracing AArch64O0PreLegalizerCombiner's real
; rule set (AArch64Combine.td) might suggest these would diverge from
; plain -global-isel here (its sub_add_reg rule, which covers this exact
; shape, is only wired into the full, non-O0 AArch64PreLegalizerCombiner)
; -- confirmed empirically WRONG by actually running both and diffing:
; this file's default (no explicit -O) invocation doesn't restrict
; -global-isel to the O0 combiner's trivial-only rule set the way that
; reasoning would assume, so the byte-diff holds for all four functions
; below. Kept here rather than a new non-byte-diff file for exactly that
; reason -- verified, not predicted, per this project's standing
; discipline.
...
; combine-identities.ll. Confirmed byte-diff clean here too, same as the
; four SubOp::fold identities above.
