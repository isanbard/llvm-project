...
;
; Unlike every other test in this suite, this one does NOT diff against
; `-global-isel` output: the oracle itself crashes on this exact input.
; `mul <16 x i8>` correctly resolves to FewerElements/scalarize on X86 (no
; packed byte-lane multiply instruction exists, confirmed via
; X86LegalizerInfo.cpp's G_MUL rule chain -- it never mentions an s8 element
; type), but the real target Legalizer's own G_BUILD_VECTOR (which it
; creates to reassemble the scalarized lanes) has no legality rule on X86,
; and the Legalizer aborts trying to legalize its own output:
...
; reasoning as every other test in this suite): the FewerElements case
; shared by AddSubLegalizePattern/BitwiseLegalizePattern/MulLegalizePattern
; (rewriteFewerElements, GMIRLegalizer.cpp) implements LegalizerHelper::
; fewerElementsVectorMultiEltType's pure-scalarize algorithm -- gmir.unmerge
; (x2, splitting each <16 x i8> operand into 16 scalar lanes) -> gmir.mul
; (x16, one per lane) -> gmir.build_vector (reassembling the lanes) -- these
; three op *shapes* (unmerge with a vector source, scalar mul, build_vector)
; only ever come from this pattern, never from GMIRImporter directly, so
; their presence here is unambiguous evidence the new legalizer code path
; ran.
