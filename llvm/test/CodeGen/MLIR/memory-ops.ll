...
;
; Verifies M3 task 1's supported subset: static-only alloca -> G_FRAME_INDEX,
; scalar (non-aggregate) gmir.load/gmir.store carrying ordering/syncscope/
; volatile/align attributes, and pointer-typed !gmir.llt values in general
; (formal args, loads, stores). See llvm/lib/CodeGen/MLIR/GMIRImporter.cpp's
; importAlloca/importLoad/importStore and MLIRToGMIRTranslator.cpp's
; convertLLT (pointer branch) / AllocaOp / LoadOp / StoreOp translateOp cases.
