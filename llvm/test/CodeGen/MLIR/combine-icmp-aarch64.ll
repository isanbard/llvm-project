...
;
; AArch64 variant of combine-identities.ll's gmir.icmp additions (gmir's
...
; downstream GICombiner at this opt level. ICmpBinOpEqOtherPattern's
; (X op Y)==X/Y->Y/X==0 family (and the sub-Y==N1 negative test) turn out
; byte-diff clean individually, but stay here too rather than being split
; into combine-identities-aarch64.ll -- same "don't fragment one coherent
; test set" call as combine-reassociate-aarch64.ll. ICmpConstAdjustPattern/
