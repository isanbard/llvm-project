...
; this one.
;
; Verifies aggregate-typed load/store, wiring GMIRImporter.cpp's
...
; or returns one directly) -- a function's own formal-parameter/return-
; type conversion is unrelated to aggregate load/store and still bails on
; anything non-scalar/pointer, so an aggregate-typed signature would make
; the whole function fall back before ever reaching importLoad/importStore.
; Storing back to %src (not just %dst) also exercises the multi-leaf value
; flowing from an aggregate load's ValueMap entry into an aggregate
; store's getOperands lookup, not just a fresh load immediately followed
; by a store.
